...
class HttpRequestHandler_Plugin:
    ...
    def post_vms(self):
        ...
        body_params = None
        try:
            body_text = self.body_bytes.decode("utf-8")
            body_params = json.loads(body_text)
        except UnicodeDecodeError:
            self.baseHttp.send_error(400, "Invalid UTF-8 in request body")
            return
        except json.JSONDecodeError:
            self.baseHttp.send_error(400, "Malformed JSON")
            return

        result,state = self._add_vm(body_params)
    ...
    def _send_txt(self, txt, content_type ='application/json', status=200):
        bs = txt.encode("utf-8")
        f = io.BytesIO(bs)

        self.baseHttp.send_response(status)
        self.baseHttp.send_header("Content-type", content_type)
        self.baseHttp.send_header("Content-Length", str(len(bs)))
        self.baseHttp.end_headers()
        if f:
            try:
                self.baseHttp.copyfile(f, self.baseHttp.wfile)
            finally:
                f.close()
    ...
    def _add_vm(self, data):
        ...
        for vm in vm_list:
            if vm["ip"] == ip and str(vm["port"]) == port:
                return json.dumps({"error": "VM已存在"}), 400

        new_id = str(max([int(vm["id"]) for vm in vm_list], default=0) + 1)
        vm = {
            "id": new_id,
            "name": name,
            "ip": ip,
        ...
        vm_list.append(vm)
        self._save_vms(vm_list)
        return json.dumps(vm), 201
...
class WebSockifyRequestHandler(WebSocketRequestHandlerMixIn, SimpleHTTPRequestHandler):
    ...
    * handler_id: A sequence number for this connection, appended to record filename
    ...
    def do_POST(self):
        '''
        HTTP Request Handle [POST]
        '''
        if self.httpHandler.do_POST() == False:
            self.send_error(405)
    ...
    def do_DELETE(self):
        '''
        HTTP Request Handle [DELETE]
        '''

        if self.httpHandler.do_DELETE() == False:
           self.send_error(405)
